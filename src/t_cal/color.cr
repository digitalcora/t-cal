...
struct TCal::Color
  ...
  def initialize(value : JSON::PullParser)
  ...
  # conversion is not exact. The "closest" color by `distance_from` is used.
  def to_ical : String
    CSS_COLORS
      .map { |name, color| {name, distance_from(color)} }
      .min_by { |_, distance| distance }[0]
  end

  # Returns the perceptual distance between this color and another.
  # Uses the "redmean" algorithm: https://www.compuphase.com/cmetric.htm
  def distance_from(other : self) : Float
    rmean = (@red.to_f + other.red) / 2
    r = (@red.to_f - other.red) ** 2
    g = (@green.to_f - other.green) ** 2
    b = (@blue.to_f - other.blue) ** 2
    Math.sqrt((((512 + rmean) * r) / 256) + 4 * g + (((767 - rmean) * b) / 256))
  end
